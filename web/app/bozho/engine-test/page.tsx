...
export default function EngineTestPage() {
	...
	useEffect(() => {
		if (currentStep) {
			const filtered = actions.filter((action) =>
				action.isAvailable(currentStep)
			);
	...
	const getStepDiffs = (index: number) => {
		...
		return {
			budget: getDiff(current.bankAccount, previous.bankAccount),
					...
					<div className="flex flex-col space-y-2">
						<div className="flex justify-between">
							...
							<div>
								...
								>
									${currentStep?.bankAccount.toFixed(2)}
								</span>
								{diffs && (
									<span className="ml-2">{diffs.budget}</span>
								)}
							</div>
						</div>
						<div className="flex justify-between">
							...
							<div>
								<span
									className={`font-medium ${getJoyColor(
										currentStep?.joy
									)}`}
								>
									{currentStep?.joy}
								...
								{diffs && (
									<span className="ml-2">{diffs.joy}</span>
								)}
							</div>
						</div>
						<div className="flex justify-between">
							...
							<tbody className="bg-white divide-y divide-gray-200">
								{caseSteps.slice(1).map((step, index) => (
									...
									>
										...
										<td className="px-6 py-4 whitespace-nowrap text-sm">
											${step.bankAccount.toFixed(2)}{" "}
											{index > 0 &&
												getDiff(
													step.bankAccount,
													caseSteps[index].bankAccount
												)}
										</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm">
											{step.joy}{" "}
