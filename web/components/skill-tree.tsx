...
export default function SkillTree() {
	...
	useEffect(() => {
		...
		const unlockedActionNames = unlockedActions
			.flat()
	...
	useEffect(() => {
		...
		nodeSelection
			...
			.attr("stroke-width", (d) =>
				canUnlock(d.id) && !d.unlocked ? 3 : 1.5
			);
	...
	const sendHint = async (question = "") => {
		...
		try {
			...
			const response = await fetch(
				`${env.NEXT_PUBLIC_DISCOVER_SERVICE}/hint`,
				{
					method: "POST",
					headers: {
			...
			if (response.ok) {
				...
				setMessages((prev) => [
					...prev,
					{ role: "assistant", content: fallbackHint },
				]);
			}
		} catch (error) {
				...
				<Card className="h-full">
					<CardHeader>
						...
						<CardDescription>
							...
							<Badge variant="outline">{availablePoints}</Badge>
						</CardDescription>
					</CardHeader>
